 ...
 * @author agung
 */
public class SubstractionTwoImages {
    public static void main(String[] args) throws Exception {
        // Read the input two imgs file and store it in a BufferedImage object
        BufferedImage img1 = ImageIO.read(new File("Z:\\citraJava\\peppers.png"));
        BufferedImage img2 = ImageIO.read(new File("Z:\\citraJava\\peppers_fake.png"));
        ...
        for (int x = 0; x < (img1.getWidth()); x++) {
            for (int y = 0; y < (img1.getHeight()); y++) {
                ...
                // Calculate the new red, green and blue values for the pixel by substraction
                int rNew = (r2 - r1);
                int gNew = (g2 - g1);
                int bNew = (b2 - b1);
                
//                Ensure that the RGB values are within the valid range (0-255)
                rNew = Math.max(0, rNew);
                gNew = Math.max(0, gNew);
                bNew = Math.max(0, bNew);
        ...
        ImageIO.write(img1, "jpg", new File("Z:\\citraJava\\SubstractionTwoImagesTest.jpg"));
    }
}